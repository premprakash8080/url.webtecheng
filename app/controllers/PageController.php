...
class Page {
        ...
        $breadcrumbSchema = [
            ...
            "itemListElement" => [
                ...
                [
                    ...
                    "position" => 2,
                    "name" => $page->name,
                    "item" => route('page', $page->seo)
                ]
            ]
        ];
        View::push('<script type="application/ld+json">'.json_encode($breadcrumbSchema, JSON_UNESCAPED_SLASHES).'</script>', 'custom')->toHeader();
    ...
    public function contact(Request $request){
        ...
        View::set('title', e('Contact Us - Get Help with URL Shortener'));

        View::set('description', e('Contact our support team for help with URL shortening, link management, QR codes, or any questions about our free URL shortener service. We\'re here to help!'));
        
        View::set('keywords', 'contact, support, help, URL shortener support, customer service');
    ...
    public function qr(){
        ...
        View::set('title', e('Free QR Code Generator - Create Custom QR Codes Online'));

        View::set('description', e('Free QR code generator tool. Create custom QR codes for URLs, text, WiFi, and more. Download QR codes in PNG, SVG, or PDF format. Track QR code scans and optimize your marketing campaigns.'));
        
        View::set('keywords', 'QR code generator, free QR code, QR code maker, custom QR code, QR code scanner, QR code creator, generate QR code');
    ...
    public function bio(Request $request){
        ...
        if($request->alias){
            ...
            if(DB::url()->where('custom', Helper::slug($request->alias))->whereRaw("(domain = '' OR domain IS NULL OR domain = '".config('url')."')")->first()){
                return Response::factory(['available' => false, 'message' => e('That alias is taken. Please choose another one.')])->json();

            }
            if(DB::url()->where('alias', Helper::slug($request->alias))->whereRaw("(domain = '' OR domain IS NULL OR domain = '".config('url')."')")->first()){
                return Response::factory(['available' => false, 'message' => e('That alias is taken. Please choose another one.')])->json();
            }
            return Response::factory(['available' => true])->json();
        }

        View::set('title', e('Bio Pages - Create Beautiful Link in Bio Pages | Free Linktree Alternative'));
        
        View::set('description', e('Create beautiful bio pages to showcase all your important links in one place. Free alternative to Linktree. Perfect for Instagram, TikTok, Twitter, and social media profiles.'));
        
        View::set('keywords', 'bio page, link in bio, linktree alternative, bio link, social media bio, instagram bio link, free bio page');
